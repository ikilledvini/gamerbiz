...
The creator portal uses the Google server-side OAuth flow with the read-only
YouTube scope. Configure these values in the Supabase project before testing:
...
In Google Cloud, enable **YouTube Data API v3** and add the callback URL above
to the OAuth client's authorized redirect URIs. While the consent screen is in
testing, every creator Google account must be listed as a test user.
