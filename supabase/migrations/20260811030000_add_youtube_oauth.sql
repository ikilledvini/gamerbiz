...
COMMENT ON TABLE public.social_oauth_states IS
  'Short-lived one-time state values used to protect creator social OAuth callbacks.';

COMMENT ON TABLE public.social_oauth_tokens IS
  'Server-only OAuth refresh tokens; never expose this table to anon or authenticated clients.';
