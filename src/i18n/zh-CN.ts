import type { Dict } from "./types";

export const zhCN: Dict = {
  meta: {
    title: "Gamerbiz — 游戏营销、人才与电竞",
    description:
      "我们通过营销活动、品牌激活与合作项目，连接游戏生态中的品牌、人才与社区。",
  },
  a11y: {
    skipToContent: "跳到主要内容",
    openMenu: "打开菜单",
    closeMenu: "关闭菜单",
    languageSwitcher: "选择语言",
    prevSlide: "上一组人才",
    nextSlide: "下一组人才",
    goToSlide: "前往第几页",
    slideStatus: "第几页",
    info: "更多信息",
  },
  nav: {
    about: "关于我们",
    brands: "品牌",
    teams: "战队",
    clients: "客户",
    talents: "达人",
    cases: "案例",
  },
  actions: {
    brand: "我是品牌方",
    creator: "我是创作者",
    close: "关闭",
    back: "返回",
    learnMore: "了解更多",
    viewCase: "查看案例",
    viewMediaKit: "查看媒体资料包",
    mediaKitSoon: "媒体资料包即将上线",
  },
  hero: {
    eyebrow: "从内容到竞技，Gamerbiz 推动游戏生态向前",
    title: "是时候升级了",
    description:
      "我们通过营销活动、品牌激活与合作项目，连接游戏生态中的品牌、人才与社区。",
    ctaPrimary: "我是品牌方",
    ctaSecondary: "认识我们的达人",
    scroll: "向下滚动探索",
    videoPlaceholder: "Gamerbiz 品牌视频",
  },
  marquee: [
    "我们深耕自己所热爱的领域",
    "战略咨询",
    "体育营销",
    "人才管理",
    "赞助合作",
    "活动执行",
    "社区运营",
    "品牌建设",
    "电子竞技",
    "内容创作者",
  ],
  stats: {
    eyebrow: "经过验证的成果",
    title: "用数字说话",
    items: [
      { value: 5, prefix: "+", label: "深耕游戏市场年数" },
      { value: 300, prefix: "+", label: "已完成项目" },
      { value: 40, prefix: "+", label: "独家签约人才" },
      { value: 70, prefix: "+", label: "服务品牌" },
    ],
  },
  about: {
    title: "我们深耕自己所热爱的领域。",
    text: "Gamerbiz 融合体育营销、人才管理、达人甄选与竞技领域的实战经验，以深度、策略和真实感打造项目。",
    mediaPlaceholder: "Gamerbiz 幕后花絮 — 图片即将上线",
    pillars: [
      {
        key: "talents",
        title: "人才",
        text: "我们代表以内容、表现力和社区影响力推动游戏文化发展的创作者与职业选手。",
      },
      {
        key: "marketing",
        title: "营销",
        text: "我们通过契合品牌目标的战略性创意营销，将影响力转化为实际成果。",
      },
      {
        key: "esports",
        title: "电子竞技",
        text: "我们具备竞技领域、人才管理和电竞职业发展方面的实战经验。",
      },
    ],
    button: "联系我们",
  },
  brands: {
    eyebrow: "品牌解决方案",
    title: "将品牌连接至游戏世界",
    description:
      "营销活动、品牌激活与合作项目，让品牌立于游戏生态中心，真实连接人才、战队与社区，并带来实际成果。",
    phrase: { top: "GAMERBIZ", mid: "你的战队。", bottom: "我们的赛场。" },
    video: "进一步了解 Gamerbiz",
    services: [
      {
        key: "strategy",
        title: "战略",
        text: "以市场洞察、精准达人选择和高效活动形式，帮助品牌在游戏领域建立定位并取得成果。",
      },
      {
        key: "influence",
        title: "影响力",
        text: "激活创作者与主播，建立真实连接，实现精准触达与游戏受众的实际影响。",
      },
      {
        key: "experiences",
        title: "体验",
        text: "通过活动、赛事与品牌激活，为品牌与社区创造值得记住的互动体验。",
      },
      {
        key: "partnerships",
        title: "合作",
        text: "连接品牌、人才与项目，在竞技和娱乐生态中创造价值、增长与机会。",
      },
    ],
  },
  teams: {
    eyebrow: "战队解决方案",
    title: "在赛场内外强化你的组织",
    description:
      "我们为战队与组织提供商务、品牌建设与表现提升方案，让你专注于真正重要的事：取胜。",
    services: [
      {
        key: "commercial",
        title: "商务",
        text: "我们为电竞组织和竞技项目匹配合适的品牌，并规划赞助、营销活动与商业化机会。",
      },
      {
        key: "scouting",
        title: "选手发掘",
        text: "我们发现有潜力的人才、阵容和项目，通过战略招募与市场洞察增强组织实力。",
      },
      {
        key: "performance",
        title: "表现提升",
        text: "我们制定竞技与组织层面的增长策略，识别赛场内外的机会。",
      },
      {
        key: "branding",
        title: "品牌建设",
        text: "我们强化组织身份、扩大行业影响力，并巩固品牌在生态中的地位。",
      },
    ],
    button: "联系 Gamerbiz",
    subject: "战队解决方案",
  },
  clients: {
    eyebrow: "他们选择信任我们",
    title: "与 Gamerbiz 并肩作战的品牌与战队",
    groupBrands: "品牌",
    groupTeams: "战队",
  },
  talents: {
    eyebrow: "我们的阵容",
    title: "不只是阵容，更是专业甄选。",
    description: "拥有活跃社区的创作者、主播与职业选手，让每位人才匹配最合适的机会。",
    card: "查看媒体资料包",
    allButton: "认识全部人才",
    badgeTalent: "Gamerbiz 签约人才",
    tooltipTalent: "由 Gamerbiz 正式代表的签约人才。",
    badgePartner: "合作创作者",
    tooltipPartner: "可与 Gamerbiz 参与精选项目的合作创作者。",
    unavailable: "媒体资料包即将上线",
    photoPlaceholder: "创作者照片",
  },
  cases: {
    eyebrow: "案例",
    title: "以表现推动转化",
    description: "真实数据，真实策略，让影响力转化为高影响力营销成果。",
    action: "查看案例",
    result: "核心成果",
    items: {
      "vittozao-brskins": {
        title: "Vittozao + Brskins",
        resultLabel: "交易总额",
        tags: ["Counter-Strike 2", "长期合作", "可追踪优惠码", "2024–2026"],
      },
      "dubblez-fate-trigger": {
        title: "Dubblez + Fate Trigger",
        resultLabel: "互动率高于平均水平",
        tags: ["YouTube Gaming", "专属视频", "Beta Launch"],
      },
      bladexzd: {
        title: "Bladexzd",
        resultLabel: "活动交付表现",
        tags: ["Gaming Creator", "高留存", "内容整合"],
      },
    },
  },
  finalCta: {
    eyebrow: "下一步",
    title: "与真正了解游戏的人一起进入游戏市场",
    button: "联系我们",
  },
  footer: {
    phrase: "从内容到竞技，Gamerbiz 推动游戏生态向前。",
    navigation: "导航",
    services: "服务",
    social: "社交媒体",
    rights: "© 2026 Gamerbiz。保留所有权利。",
    privacy: "隐私政策",
    terms: "使用条款",
    legalUnavailable: "该文件正在准备中，敬请期待。",
  },
  brandModal: {
    eyebrow: "GAMERBIZ · 品牌",
    title: "聊聊",
    titleHighlight: "你的项目？",
    description: "请填写以下信息，我们的团队将在 24 小时内与您联系。",
    name: "您的姓名",
    namePlaceholder: "例如：张伟",
    company: "公司",
    companyPlaceholder: "公司名称",
    email: "电子邮箱",
    emailPlaceholder: "email@company.com",
    whatsapp: "WhatsApp",
    whatsappPlaceholder: "+86 138 0000 0000",
    help: "我们可以如何帮助您？",
    helpPlaceholder: "请简单介绍您的项目或目标……",
    submit: "发送消息 →",
    success: "信息已准备完成，我们的团队将尽快与您联系。",
    optional: "选填",
  },
  creatorModal: {
    eyebrow: "GAMERBIZ · 创作者",
    title: "加入",
    titleHighlight: "我们的团队",
    description: "请填写以下信息，我们的团队将尽快与您联系。",
    name: "您的姓名",
    namePlaceholder: "例如：张伟",
    email: "电子邮箱",
    emailPlaceholder: "email@email.com",
    whatsapp: "WhatsApp",
    whatsappPlaceholder: "+86 138 0000 0000",
    platforms: "您在哪些平台创作内容？",
    other: "其他",
    type: "您创作哪类内容？",
    typePlaceholder: "例如：游戏实况与娱乐视频",
    profiles: "我们可以在哪里找到您？",
    profilesPlaceholder: "例如：@用户名或您的主页链接",
    submit: "提交申请 →",
    success: "申请已准备完成，我们的团队将尽快与您联系。",
    optional: "选填",
  },
  validation: {
    required: "此字段为必填项。",
    email: "请输入有效的电子邮箱。",
    platform: "请至少选择一个平台。",
    profiles: "请至少填写一个用户名或主页链接。",
    generic: "暂时无法完成操作，请检查填写内容后重试。",
  },
};
