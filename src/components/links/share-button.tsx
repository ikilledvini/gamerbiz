...
export function ShareButton({ className }: { className?: string }) {
  ...
  return (
    <button
      ...
      className={`flex h-11 w-11 items-center justify-center rounded-full border border-border bg-black text-foreground outline-offset-2 duration-200 ease-out [transition-property:transform,color,border-color] focus-visible:outline focus-visible:outline-2 focus-visible:outline-foreground active:scale-[0.96] [@media(hover:hover)and(pointer:fine)]:hover:border-primary [@media(hover:hover)and(pointer:fine)]:hover:text-primary ${className ?? ""}`}
    >
